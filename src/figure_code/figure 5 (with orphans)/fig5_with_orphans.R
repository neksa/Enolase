...
ps.options(
    ...
    paper = "special",
    pointsize=9)
postscript("/Home/bccs/sofija/enolase/clustering/figures/fig5_with_orphans.eps",
...
d <- read.table("/Home/bccs/sofija/enolase/clustering/active_clusters_with_orphans.csv",header=T,sep="\t")
d1 <- na.omit(sapply(d[3], as.numeric))
plot(d1, type="l", col=cols[1],xlab="number of clusters",ylab="orphans")
abline(lsfit(1:10, 1:10),col = "gray60", lty=2)
dev.off()
